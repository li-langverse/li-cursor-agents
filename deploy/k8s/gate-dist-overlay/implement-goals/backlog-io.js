...
const MARKDOWN_TODO_RE = /- id: (\S+)\n\s+content: (?:"([^"]+)"|([^\n]+))\n\s+status: (\w+)/g;
const PLAN_TODO_RE = /- id: (\S+)\n\s+content: "?([^"\n]+)"?\n\s+status: (\w+)/g;
export function parseBacklogTodos(text, format) {
    const fmt = format ?? (text.includes("docs/superpowers/plans/") ? "plan_yaml" : "markdown_todos");
    if (fmt === "plan_yaml") {
...
export function loadBacklogTodos(goal) {
    ...
    try {
        ...
        const format = goal.backlog_format ??
...
export function pickNextBacklogTodo(todos, options) {
    const completed = options?.completedIds ?? new Set();
    const preferInProgress = options?.preferInProgress ?? true;
    if (options?.todoId) {
        return todos.find((t) => t.id === options.todoId) ?? null;
    }
    const open = todos.filter((t) => (t.status === "pending" || t.status === "in_progress") && !completed.has(t.id));
    if (!open.length)
        return null;
    open.sort((a, b) => {
...
export function buildImplementGoalMarkdown(goal, todo, gatesPath) {
    const lines = [
        "---",
...
export function markBacklogTodoDone(goal, todoId) {
    ...
    const pattern = new RegExp(`(- id:\\s*${todoId.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s*\\n\\s+content:[^\\n]*\\n\\s+status:)\\s*\\w+`, "m");
...
//# sourceMappingURL=backlog-io.js.map