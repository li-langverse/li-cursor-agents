...
BRANCH="${LI_GOAL_BRANCH:-cursor/li-research-r1}"
BRANCH_SCHEMA="${LI_GOAL_BRANCH_SCHEMA:-cursor/li-research-r1}"
GOAL_FILE_REL="${LI_GOAL_FILE:-data/goal-directed-sprints/wp-li-research-r1-product.md}"
...
seed_goal() {
  ...
  if [[ -f /config/wp-li-research-r1-product.md ]]; then
    cp -f /config/wp-li-research-r1-product.md "${AGENTS_ROOT}/data/goal-directed-sprints/"
  fi
}
...
resolve_goal_file() {
  ...
  if [[ -f /config/wp-li-research-r1-product.md ]]; then
    echo "/config/wp-li-research-r1-product.md"
    return 0
  fi
  return 1
}
