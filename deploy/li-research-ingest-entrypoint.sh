...
BRANCH="${LI_GOAL_BRANCH:-cursor/li-research-public-index}"
GOAL_FILE_REL="${LI_GOAL_FILE:-data/goal-directed-sprints/wp-li-research-public-index.md}"
...
seed_goal() {
  ...
  if [[ -f /config/wp-li-research-public-index.md ]]; then
    cp -f /config/wp-li-research-public-index.md "${AGENTS_ROOT}/data/goal-directed-sprints/"
  fi
}
...
resolve_goal_file() {
  ...
  if [[ -f /config/wp-li-research-public-index.md ]]; then
    echo "/config/wp-li-research-public-index.md"
    return 0
  fi
  return 1
}
