...
LOOP_SLEEP="${LI_WORLD_STUDIO_GUI_DEMO_RECORDER_LOOP_SLEEP_SEC:-${LI_GOAL_LOOP_SLEEP_SEC:-60}}"
...
while true; do
  ...
  echo "world-studio-gui-demo-recorder-entrypoint: loop stopped without completion (exit $rc) â€” retry in ${LOOP_SLEEP}s" >&2
  sleep "$LOOP_SLEEP"
done
