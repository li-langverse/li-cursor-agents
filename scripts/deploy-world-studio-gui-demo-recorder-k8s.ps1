# Deploy li-world-studio-gui-demo-recorder goal-directed agent on homelab engine cluster.
param(
...
Write-Host "==> kubectl apply li-world-studio-gui-demo-recorder (namespace=$Namespace)"
...
$extra = @{
    "entrypoint.sh" = (Join-Path $Root "deploy\world-studio-gui-demo-recorder-entrypoint.sh")
}
. (Join-Path $Root "scripts\Invoke-K8sGoalLoopBundle.ps1") `
    -Root $Root -Namespace $Namespace -ConfigMapName "li-world-studio-gui-demo-recorder-bundle" `
    -ExtraFiles $extra
...
Write-Host "  kubectl -n $Namespace get pods -l app=li-world-studio-gui-demo-recorder -w"
Write-Host "  kubectl -n $Namespace logs deploy/li-world-studio-gui-demo-recorder --tail=80 -f"
