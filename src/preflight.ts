...
function readBriefingBundle(benchmarksRoot: string): PreflightBundle {
...
function preflightTimeoutMs(): number {
  const n = Number(process.env.LI_MAINTENANCE_PREFLIGHT_TIMEOUT_MS ?? 600_000);
  return Number.isFinite(n) && n >= 30_000 ? n : 600_000;
}

/** Non-blocking preflight for maintenance lane; abort kills the python child. */
export async function runPreflightAsync(
  ...
  if (signal?.aborted) {
    throw new Error("runPreflightAsync: aborted");
  }

  const args = skipSlow ? ["--skip-slow"] : [];
  const exitCode = await new Promise<number>((resolve, reject) => {
    const child = spawn("python3", [py, ...args], {
      cwd: benchmarksRoot,
      env: {
        ...process.env,
    ...
    const timeout = setTimeout(() => {
      ...
      reject(new Error(`runPreflightAsync: timeout after ${preflightTimeoutMs()}ms`));
    }, preflightTimeoutMs());
    ...
    const onAbort = () => {
      ...
      reject(new Error("runPreflightAsync: aborted"));
    };
  ...
  const bundle = readBriefingBundle(benchmarksRoot);
  bundle.runs = {
    agent_briefing: { exit_code: exitCode },
  };
  return bundle;
}
