...
export function resolveBranchFromGoalText(text: string): string | undefined {
...
export function resolvePlanRelFromGoalText(text: string): string | undefined {
  const link = text.match(PLAN_LOOP_LINK)?.[1]?.trim();
  if (!link) return undefined;
  return link.replace(/^\.\.\//, "");
}

export function resolveBranchFromGoalFile(goalFile: string): string | undefined {
  const path = resolve(goalFile);
  const text = readFileSync(path, "utf8");
  return resolveBranchFromGoalText(text);
}

export function resolvePlanPathFromGoalFile(goalFile: string, cwd?: string): string | undefined {
  const path = resolve(goalFile);
  const text = readFileSync(path, "utf8");
...
export function inferPlanPrefixFromGoalFile(goalFile: string): string | undefined {
  const name = basename(goalFile, ".md");
  if (name.includes("product-visual")) return "wsv-w";
  if (name.includes("gui-polish")) return "wsp-w";
  if (name.includes("gui-library") || name === "world-studio-gui-library") return "wsg-w";
  if (name.includes("master-plan")) return "wsm-w";
  return undefined;
}
