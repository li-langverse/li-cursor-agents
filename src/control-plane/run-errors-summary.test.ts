...
test("summarizeRunErrors counts unregistered_running_reconciled as stale bookkeeping", () => {
  const summary = summarizeRunErrors([
    {
      run_id: "u-1",
    ...
    },
    {
      run_id: "u-2",
      agent_id: "proof_gap_researcher",
      started_at: "2026-05-28T10:10:00.000Z",
      finished_at: "2026-05-28T10:15:00.000Z",
      status: "error",
      error: "unregistered_running_reconciled",
    },
    {
      ...
      finished_at: "2026-05-28T11:05:00.000Z",
      status: "error",
      error: "SDK run status: error",
    },
  ]);

  assert.equal(summary.stale_reconcile_count, 2);
  assert.equal(summary.real_error_count, 1);
  const unreg = summary.categories.find((c) => c.category === "unregistered_running_reconciled");
  assert.ok(unreg);
  assert.equal(unreg!.count, 2);
});
