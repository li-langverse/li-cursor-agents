...
| Tool | When |
|------|------|
| `close_gitlab_issue` | Decision is **close** (duplicate, already on main, spam, wontfix, superseded, not actionable, stale) |

### Close workflow (mandatory)

1. Investigate the issue body, labels, linked MRs, and codebase.
2. If it should close, call **`close_gitlab_issue`** with:
   - `repo`, `number`
   - `reason`: `already_implemented` | `duplicate` | `wontfix` | `spam` | `superseded` | `not_actionable` | `stale_no_response`
   - `summary`: one line
   - `evidence`: MR !, file on main, duplicate of #N, etc.
3. Verify the tool response has **`"closed": true`**. If not, fix evidence and retry once.
4. **Do not** close via GitLab UI, `glab issue close`, or prose-only recommendations.
...
- Finish with "should close" without calling `close_gitlab_issue`
