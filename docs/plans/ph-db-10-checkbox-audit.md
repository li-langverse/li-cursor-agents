...
| [x] | `ControlPlaneStore` includes `"lidb"` | `src/db/client.ts` `supabase \| disk \| lidb` |
| [x] | `assertStoreReady()` for lidb | `lidbStoreReady()` — `LI_LIDB_URL` or `LI_LIDB_MOCK=1` |
| [x] | `persist.ts` liorm paths | `src/db/lidb-persist.ts` stub hooks + disk mirror |
...
**Counts:** 6 closed, 2 open (PH-DB-10 migration checklist only).
...
3. **Then:** backfill script + un-skip e2e `test.todo` rows when `LI_LIDB_URL` engine accepts schema.  
4. **Blocked on:** lidb engine + schema parity with `supabase/migrations/` (real liorm execute).
