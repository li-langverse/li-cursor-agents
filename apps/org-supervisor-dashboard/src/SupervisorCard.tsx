  ...
  return (
    <article className={`card${active ? " active" : ""}`}>
              ...
              <tbody>
                {supervisor.activeClaims.map((row, i) => (
                  <tr key={i}>
                    ...
                    <td className={statusClass(row.status)}>{String(row.status ?? "ิว๖")}</td>
                    <td>{String(row.workerId ?? "ิว๖")}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="audit-list">
            {audits.slice(0, 8).map((row, i) => (
              <div className="audit-item" key={i}>
                ...
                <div>{refLabel(row)} -ภ worker {String(row.workerId ?? "ิว๖")}</div>
      ...
      <footer className="kubectl">
        <div className="meta">
          PVC coordination: <code>{supervisor.activeFile}</code> -ภ KUBECONFIG=
